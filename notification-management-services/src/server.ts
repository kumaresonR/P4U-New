...
const PORT = parseInt(process.env.SERVER_PORT || '8088', 10);
...
app.get('/', (_req: Request, res: Response) => {
  res.json({
    message: 'P4U Notification Service',
    basePath: '/api/v1/notifications',
    publicHealth: '/api/v1/notifications/public/health',
  });
});
    ...
    app.listen(PORT, async () => {
      try {
        await discovery.register({
          ...
          host: SERVICE_HOST,
          port: PORT,
          healthCheckUrl: `http://${SERVICE_HOST}:${PORT}/api/v1/notifications/public/health`,

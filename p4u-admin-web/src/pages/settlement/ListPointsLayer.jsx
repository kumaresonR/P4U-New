  ...
  const load = useCallback(async () => {
    ...
    try {
      const [sRes, vRes] = await Promise.all([
        listPointsSettlements({ limit: 100, offset: 0 }),
          ...
          <div className='table-responsive scroll-sm'>
            <table className='table bordered-table sm-table mb-0 text-nowrap'>
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr>
                    <td colSpan='9' className='text-center py-4'>No point settlements found.</td>
                  </tr>
                ) : (
                  filtered.map((item, index) => {
                    ...
                    return (
                      <tr key={item.id}>
                        ...
                        <td>{v.mobile}</td>
                        <td className='fw-semibold text-primary-main'>{item.amount != null ? `${item.amount} Pts` : "—"}</td>
...
export default ListPointsLayer;
